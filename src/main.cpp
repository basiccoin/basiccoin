...
#if defined(NDEBUG)
#error "Basiccoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Basiccoin Signed Message:\n";
...
bool CTxOut::IsDust() const {
    // Basiccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees) {
    ...
    nSubsidy >>= (nHeight / 25000); // Basiccoin: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    // Basiccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const {
    ...
    // Basiccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlockIndex::IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned int nRequired, unsigned int nToCheck) {
    // Basiccoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex() {
    if (fTestNet) {
        ...
        hashGenesisBlock = uint256("0x657a808e5be1f5f91dd429b18ed52cccf0bad59dd7f69dc0c39121bebaa8f5db");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("03b5d38a1e262c21922f2ffa48375eb583eeb3e45c24e6fb367167e11eff43f80f") << OP_CHECKSIG;
...
unsigned char pchMessageStart[4] = {0xfb, 0xc0, 0xb6, 0xdb}; // Basiccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BasiccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BasiccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BasiccoinMiner(CWallet *pwallet) {
    printf("BasiccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("basiccoin-miner");
    ...
    try {
        loop{
            ...
            printf("Running BasiccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet) {
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BasiccoinMiner, pwallet));
}
